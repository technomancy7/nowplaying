...
while True: 
    ...
    if details != last_details:
        d = RPC.update(state=f"Playing in {displayer}", details=f"{details}", large_text="Designed by Technomancer", large_image="infinity-transparent", small_text=displayer, small_image=icon, start=time.time(), buttons=[{"label": "Source", "url": "https://github.com/technomancy7/nowplaying.py"}])
        update = d['data']
